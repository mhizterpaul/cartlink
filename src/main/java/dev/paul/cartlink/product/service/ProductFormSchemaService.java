...
public class ProductFormSchemaService {
    ...
    public Optional<ProductFormSchema> getByTypeId(String typeId) {
        // Search for schema where schemaJson->>'typeId' = :typeId (Postgres JSONB query)
            ...
            .filter(schema -> {
                try {
                    com.fasterxml.jackson.databind.JsonNode node =
                        new com.fasterxml.jackson.databind.ObjectMapper().readTree(schema.getSchemaJson());
                    return node.has("typeId") && typeId.equals(node.get("typeId").asText());
                } catch (Exception e) {
                    return false;
                }
            })
    ...
    public ProductFormSchema saveOrUpdate(String productType, String schemaJson, String source, String status, String usageStatsJson) {
